...
**缺陷平衡铁律：真实漏洞（FN）一条不能丢，误报（FP）能减就减。** 「不增加漏报」不等同于「不能减少 findings」——只要能证明某条 finding 是误报（如桥接假连通、跨文件错配、宽 sink 误标、遮蔽错配），就该砍，总数下降是目标不是禁忌。唯一硬约束是真实漏洞召回：任何收窄规则 / 切断桥接 / 位置门控的改动，都必须先在既有基准（vfa / flask-xss / vampi / demo-java 及探针样例）上证明「真实漏洞命中一条不少」再落地；做不到就保留过近似，靠规则更精确而非靠砍路径。

**内存铁律：跑任何大型命令（全量扫描 / 回归 / 探针生成器）前先评估内存。** 本机约 1.6GB RAM，曾因完整 CLI + 大 `--max-findings` 当场 OOM。OWASP 全量须分块扫描（275 文件/块 × 10）；大型性能测量用插桩计时（`.perf_probe_bench.py`，纯建图无 BFS 无报告写出）；长产物先落仓库根持久目录（如 `perf_probes/`），不要依赖 `/tmp`（重启会清）。
...
- 全量误报面排查（A–F 分类：sink 打标过宽 / 规模乘积 / 汇总性能 / 桥接放大 / def-use 遮蔽错配）见 `docs/误报面清单.md`。
